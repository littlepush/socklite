...
// If and only if this is an integration version inside the socklite library
#ifdef SOCK_LITE_INTEGRATION_LOG
...
namespace cpputility {
	...
	cp_logger_specifical_character::operator bool () const {
		return (_sc != '\0');
	}
    cp_logger_specifical_character::cp_logger_specifical_character( char c ) : _sc(c) { }
    cp_logger_specifical_character::cp_logger_specifical_character( bool eol ) : _sc('\0') {
    	if ( eol == false ) _sc = '\n';
    }
    char cp_logger_specifical_character::operator() (void) const {
    	return _sc;
    }
    ...
    cp_logger & operator << <cp_logger_specifical_character> ( cp_logger& logger, const cp_logger_specifical_character & item ) {
        ...
        // Write the log
        cp_log(logger.lv_, "%s", logger.oss_.str().c_str());
        logger.oss_.str("");
        logger.oss_.clear();
	...
	cp_logger cp_logger::emergancy(log_emergancy);
	cp_logger cp_logger::alert(log_alert);
    cp_logger cp_logger::critical(log_critical);
    cp_logger cp_logger::error(log_error);
    cp_logger cp_logger::warning(log_warning);
    cp_logger cp_logger::notice(log_notice);
    cp_logger cp_logger::info(log_info);
    cp_logger cp_logger::debug(log_debug);

    // End of current log line
    cp_logger_specifical_character cp_logger::endl('\0');
    cp_logger_specifical_character cp_logger::newline('\n');
    cp_logger_specifical_character cp_logger::backspace('\b');
    cp_logger_specifical_character cp_logger::tab('\t');
}
#endif
